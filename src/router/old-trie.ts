...
class TrieNode {
  ...
  pattern: string;
  path: string;
  methodMap: Map<string, Function>; // O(1) method lookup
 ...
 class Trie {
  ...
  search(path: string, method: HttpMethod) {
    ...
    let collected: Function[] = [...this.globalMiddlewares];
...
// anyway deprecated so types doesnt matter
// Implementation
export class OldTrieRouter implements Router {
